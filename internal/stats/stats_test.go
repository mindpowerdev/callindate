...
func TestBuildReport(t *testing.T) {
	...
	if a.Name != "Плавание" || a.Planned != 2 || a.Attended != 1 || a.Missed != 1 || a.Spent != 4000 {
		t.Fatalf("неверная статистика: %+v", a)
	}

	cost, ok := a.CostPerVisit()
	if !ok || cost != 4000 {
		t.Fatalf("CostPerVisit = %v, %v, want 4000, true", cost, ok)
	}

	if report.TotalSpent != 4000 || report.TotalAttended != 1 {
